    ...
    if short_seq in long_seq:
        ...
        if short_seq[0:step] not in long_seq:
            for m in range(1,len(short_seq)- step + 2):
                ...
                sys.exit('"%s" "%s" overlap less than 11bp' % (long_seq, short_seq))
                merged_seq = long_seq
        else:
            for m in range(1, len(short_seq)- step + 2):
...
def merge_hairpin_list(seq_list):
    merged_seq = ''
...
if __name__ == '__main__':
                    ...
                    if m in  sample_index_dict:
                        ...
                        if each_col != '-':
                            ...
                            for each_col_info in each_col_list:
                                hairpin_seq_list.append(miRNA_seq_dict[each_col_info][sample_id][-1])
                each_hairpin_merged_seq = merge_hairpin_list(hairpin_seq_list)
                hairpin_fa_dict[novel_id] = each_hairpin_merged_seq
    ...
    with open(hairpin_fa, 'w') as hairpin_fa_info:
        for each_novel in hairpin_fa_dict:
            hairpin_fa_info.write('>%s\n%s\n' % (each_novel, hairpin_fa_dict[each_novel]))
