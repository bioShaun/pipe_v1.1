    ...
    with open(kegg_blast,'r') as kegg_blast_info :
        for eachline in kegg_blast_info :
            ...
            if query_id not in diff_blast_dict :
                ...
                if diff_blast_dict[query_id][-2] == each_tr_blast[-2] and diff_blast_dict[query_id][-1] < each_tr_blast[-1]:
                    diff_blast_dict[query_id] = each_tr_blast[1:]
...
def treat_KEGG_table(kegg_out):
    kegg_out_dir,kegg_out_name = os.path.split(kegg_out)
    kegg_tmp_file = os.path.join(kegg_out_dir,'tmp.%s' % kegg_out_name)
    os.system('mv %s %s' % (kegg_out,kegg_tmp_file))
    kegg_out_info = open(kegg_out,'w')
...
def enrich_bar_plot(go_out,kegg_out):
    rbin = PLOT_R_BIN
    script = SIMPLE_PLOT_SP
    out_dir,enrich_name = os.path.split(go_out)
    compare_name = enrich_name.split('.')[0]
    os.system('{rbin}/Rscript {script} {compare_name} {go_out} {kegg_out} {out_dir}'.format(**locals()))
...
def main() :
    ...
    for each_compare in compare_list :
        ...
        ## enrichment plot
        enrich_bar_plot(go_out,kegg_out)
