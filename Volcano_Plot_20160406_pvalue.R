...
dpa_results <- diff_file_info[,c("logFC","PValue"),with =F]
dpa_results$logFDR <- -log10(dpa_results$PValue)
dpa_results$color <- "blue"

for(i in 1:dim(dpa_results)[1]){
  if(dpa_results$logFC[i] > lgfc & dpa_results$PValue[i] < p)
      dpa_results$color[i] <- "red"
  else if(dpa_results$logFC[i] < -(lgfc) & dpa_results$PValue[i] < p)
...
logFDR_limit <- ifelse(logFDR_limit<10,10,logFDR_limit)
...
p <- ggplot(dpa_results2,aes(logFC,logFDR,colour = color))+geom_point(size = .6)+
  ...
  geom_vline(xintercept = lgfc,lty = 4,size = .45)+
  xlab("logFC")+ylab("-log10(Pvalue)")+ggtitle(name)+
  scale_y_continuous(breaks = c(0,1.3,3,10),limits = c(0,logFDR_limit))+
